...
def visualization(image, box, display_str='', thickness=4, use_normalized_coordinates=True):
...
def main():
    ...
    while ret:
        ...
        print("will find hand is %d"%num_detections)
        for stepss in range(0,num_detections):
            print("the stepss is %d"%stepss)
            if output_dict['detection_scores'][0][stepss] > 0.3:
                ...
                x = sz[0]
                y = sz[1]
                roi = org_img[int(x * box0[0]): int(x * box0[2]), int(y * box0[1]): int(y * box0[3])]
...
if __name__ == '__main__':
    detec_model_path = 'frozen_inference_graph.pb'
    detec_label_map_path = 'human_hand_label_map.pbtxt'
    recog_model_path = 'six_graph-z.pb'
    recog_label_path = 'six_labels-z.txt'

    recog_model_path = 'intermediate.pb'
